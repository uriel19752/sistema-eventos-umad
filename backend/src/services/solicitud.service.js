...
import { enviarAlertaNuevaSolicitud, enviarAlertaCancelacionTardia, enviarCorreoAprobacion, enviarCorreoCancelacion, enviarCorreoConfirmacionSolicitud, enviarNotificacionProveedor, } from "./mailService.js";
import { actualizarEventoSolicitud, construirDescription, crearEventoSolicitud, eliminarEventoSolicitud, } from "./googleCalendarEvent.service.js";
    ...
    prisma.usuario
        ...
        if (creador?.email) {
            enviarCorreoConfirmacionSolicitud({
                ...
                horaInicio: formatTime(solicitud.horaInicio),
                responsableNombre: solicitud.responsableNombre,
            }).catch((e) => console.error("[MAIL] Error enviando confirmación al solicitante", e));
        }
        else {
            console.log("[MAIL] Creador sin email. No se envió confirmación.");
        }
    })
        .catch((e) => console.error("[MAIL] Error buscando creador para confirmación", e));
    prisma.usuario
        ...
        for (const ap of proveedoresCancelacion) {
            if (ap.proveedor.email) {
                enviarNotificacionProveedor('cancelacion', {
                    ...
                    contacto: solicitudActual.contacto ?? '',
                }).catch((e) => console.error(`[MAIL] Error al notificar cancelación a ${ap.proveedor.email}:`, e));
            }
        }
        return resultado;
    }
...
export async function asignarProveedores(solicitudId, proveedorIds) {
