...
function buildDateFilter(fechaInicio, fechaFin) {
    const filter = {};
    if (fechaInicio)
        filter.gte = new Date(fechaInicio);
    if (fechaFin)
        filter.lte = new Date(fechaFin + "T23:59:59.999Z");
    return Object.keys(filter).length > 0 ? { fechaSolicitud: filter } : {};
}
export async function obtenerDashboardEstadisticas(filtros) {
    ...
    const wherePorPlantel = { ...dateWhere };
    if (filtros?.institucion && filtros.institucion !== "todos") {
        wherePorPlantel.institucion = { nombre: filtros.institucion };
    }
    const wherePorInstitucion = { ...dateWhere };
    if (filtros?.plantel && filtros.plantel !== "todos") {
    ...
    sqlConditions.push(Prisma.sql `se.fecha_solicitud >= ${new Date(startDate.getFullYear(), startDate.getMonth(), 1)}::timestamp`);
    sqlConditions.push(Prisma.sql `se.fecha_solicitud < ${new Date(endDate.getFullYear(), endDate.getMonth() + 1, 1)}::timestamp`);
    ...
    if (!filtros?.fechaInicio && !filtros?.fechaFin) {
        ...
        const [currentGroup, previousGroup] = await Promise.all([
            prisma.solicitudEvento.groupBy({
                ...
                where: {
                    ...baseWhere,
                    fechaSolicitud: { gte: startOfCurrentMonth, lt: startOfNextMonth },
                },
                _count: true,
            }),
            prisma.solicitudEvento.groupBy({
                ...
                where: {
                    ...baseWhere,
                    fechaSolicitud: { gte: startOfPreviousMonth, lt: startOfCurrentMonth },
                },
                _count: true,
            }),
        ]);
        const buildCountMap = (groups) => {
            const map = {
                Pendiente: 0,
                Aprobado: 0,
                Completada: 0,
        ...
        const calcVariacion = (actual, anterior) => anterior === 0 ? 100 : Math.round(((actual - anterior) / anterior) * 100);
        tendencias = {
    ...
    const plantelLider = porPlantel.length > 0
        ? {
    ...
    const institucionLider = porInstitucion.length > 0
        ? {
        ...
        : { nombre: "N/A", porcentaje: 0 };
    const mesMasActivo = porMes.length > 0
        ? (() => {
    ...
    const tipoTendencia = pctTendencia > 5
        ? "crecimiento"
        : pctTendencia < -5
            ? "decrecimiento"
            : "estable";
    ...
    const prevSolicitudWhereEncuesta = {
        ...prevWhere,
        ...(!esAdmin && filtros?.usuarioId ? { usuarioId: filtros.usuarioId } : {}),
    };
    ...
    const currentGlobalAvg = (promediosFormateados.puntualidad +
        promediosFormateados.calidadTecnica +
        promediosFormateados.atencionStaff +
        promediosFormateados.satisfaccionGral) / 4;
    const prevGlobalAvg = prevPromedios._count.id > 0
        ? ((prevPromedios._avg.puntualidad ?? 0) +
            ...
            (prevPromedios._avg.satisfaccionGral ?? 0)) / 4
        : 0;
    const variacionCSAT = {
