...
function accionesSolicitudHtml(solicitudId) {
    const FRONTEND_URL = process.env.FRONTEND_URL || 'http://localhost:5173';
    return `
...
export async function enviarCorreoConfirmacionSolicitud(datos) {
...
export async function enviarNotificacionProveedor(tipo, datos) {
    if (tipo === 'asignacion') {
        const cuerpo = `
