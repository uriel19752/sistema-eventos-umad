...
export async function generarRecordatoriosAutomatizados() {
    ...
    const inicioManana = new Date(manana.getFullYear(), manana.getMonth(), manana.getDate());
    ...
    for (const n of notisHoy) {
        ...
        if (match)
            yaNotificados.add(`${n.usuarioId}:${match[1]}`);
    }
    for (const solicitud of solicitudesManana) {
        ...
        for (const usuarioId of uniqueTargets) {
            if (yaNotificados.has(`${usuarioId}:${solicitud.folio}`))
                continue;
            await crearNotificacion(usuarioId, "⏰ Recordatorio de Evento", `El evento "${solicitud.nombreEvento}" (folio ${solicitud.folio}) está programado para mañana. ¡No olvides revisar los detalles!`);
        }
    }
}
