...
function renderEstrellas(doc, rating, x, y, size) {
    for (let i = 0; i < 5; i++) {
        const sx = x + i * (size + 4);
...
export async function exportarPDF(req, res) {
    try {
        ...
        res.setHeader("Content-Disposition", 'attachment; filename="TigreTrack_Reporte_Calidad.pdf"');
        doc.pipe(res);
        // ── PORTADA ──
        ...
        const filtros = [];
        if (plantel)
            filtros.push(`Plantel: ${plantel}`);
        if (institucion)
            filtros.push(`Institución: ${institucion}`);
        if (filtros.length) {
            doc
                .fontSize(9)
                .fillColor("#94A3B8")
        ...
        if (p) {
            ...
            doc
                .fontSize(12)
                .fillColor(color)
                .text(pct >= 85 ? "Excelente" : pct >= 70 ? "Regular" : "Crítico", cardX + 160, cardY + 30, { width: 120 });
            doc
                .fontSize(9)
                .fillColor("#64748B")
                .text(`Promedio general basado en ${p.totalEncuestas} evaluaciones`, cardX + 160, cardY + 52, { width: 120 });
            doc
                .fontSize(9)
                .fillColor("#334155")
...
export async function exportarExcel(req, res) {
    try {
        ...
        res.setHeader("Content-Disposition", 'attachment; filename="TigreTrack_Reporte_Calidad.xlsx"');
