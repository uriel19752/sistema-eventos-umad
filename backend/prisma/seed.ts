...
async function main() {
  ...
  const usuarios = await prisma.usuario.createMany({
    data: [
      { correo: '4ngel3duardofongestrada@gmail.com', password: hashAdmin, rol: Rol.ADMIN },
  ...
  const proveedores = await prisma.proveedor.createMany({
    data: [
      { nombre: 'Audio y Sonido Profesional Puebla', especialidad: 'Sonido, iluminación y audio profesional', activo: true },
      { nombre: 'Lonas y Estructuras Atlixco', especialidad: 'Lonas, estructuras y material publicitario', activo: true },
      { nombre: 'Catering Express', especialidad: 'Servicio de banquetes y coffee break', activo: true },
    ],
  })
